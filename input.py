    ...
    if datetime.datetime.now().weekday() < 5 and datetime.datetime.now().hour >= 7 and datetime.datetime.now().hour < 17:
        ...
        for projectid, projectname,inputid, inputdate, reqby, inptype, subject, assto, due, closed, closedate, reqtext, notifiedDate in noNotifications:
            if notifiedDate == None:
                ...
                try:
                    if subject in ['01TE','QTPC', 'QTPH', '05TE', '07TE', '08TE', '11PH', '13TE', 'PM02', 'PM03', 'PM04', 'PM05', 'PM06', 'PM07', 'PM09', 'PM10', 'PM11', 'PM12', 'PM13', 'PM15', 'PM17', 'PM18', 'PM20', 'PM23', 'PM24', 'PM25', 'PM26', 'PM28', 'PM29' ] or 'F8511-2 C' in reqtext:
...
def Reminder():
    ...
    for projectid, projectname, inputid, *others in noNotifications:
        ...
        if datetime.now() > lastNotifiedDate + timedelta(days=7):
            # print("send reminder")
            notification = '''Action items reminder. Please review and take appropriate and timely action. \nAction id: %s \nRequest date: %s \nRequest: %s \nProject: %s - %s\n\nIf you have any questions please contact the quality manager.''' % (inputid, others[0], others[8], projectid, projectname)
            # print(notification)
            # print(shortreq)
