...
def Reminder():
    ...
    for projectid, projectname, inputid, *others in noNotifications:
        ...
        if datetime.now() > lastNotifiedDate + timedelta(days=7):
            ...
            # print(notification)
            utils.sendMail(to_email=["tim.kent@ci-aviation.com"], cc_email="tim.kent@ci-aviation.com", subject=f"Action Item Notification: {inputid} - {shortreq}", message=notification, from_email="tim")
